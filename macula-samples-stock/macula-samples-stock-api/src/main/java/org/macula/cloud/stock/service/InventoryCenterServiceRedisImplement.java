...
@Service
@AllArgsConstructor
public class InventoryCenterServiceRedisImplement implements InventoryCenterService {
	...
	@Override
	public InventoryResponse fulfilment(InventoryRequest request) {
		...
		try {
			...
			for (ProductQuantity d : details) {
				...
				List<String> fulfilmentCodes = InventoryUtils.resolveValuePath(d.getFulfillmentWhCode());
				for (String fulfilmentCode : fulfilmentCodes) {
					values.add(InventoryUtils.getStockRedisKey(request.getAccDate(), d.getProductCode(), fulfilmentCode, d.getFulfillmentLocCode()));
				}
				keys.add(String.valueOf(fulfilmentCodes.size() + 2));
			}
			keys.add(InventoryUtils.getUniqueIndexKey(request));
	...
	public InventoryResponse updateFulfillmentStockCode(InventoryRequest request) {
		...
		request.getDetails().forEach(d -> {
			...
			String routeKey = d.getRouteStockRedisKey(accDate, deliveryType);
			if (stringRedisTemplate.hasKey(routeKey)) {
				String fulfilmentPath = (String) stringRedisTemplate.opsForHash().get(routeKey, "fulfilmentPath");
				if (StringUtils.isNotBlank(fulfilmentPath)) {
					d.setFulfillmentWhCode(fulfilmentPath);
				}
			}
		});
		return InventoryResponse.success(request);
