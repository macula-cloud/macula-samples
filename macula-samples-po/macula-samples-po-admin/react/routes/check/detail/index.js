...
const Detail = observer(() => {
  ...
  return (
    <TabPage>
      <Header backPath={`/order/check-list${location.search}`} backText="返回对账单列表">
        <Button
          icon="refresh"
          onClick={async () => {
          ...
          >
            ...
            <Column
              ...
              renderer={({text, record}) => {
                if (record.get('gbssSapDocNo') !== record.get('omsSapDocNo')) {
                  return (<span
                    style={{color: '#ff3fa4'}}>{`${(text == null || text == '') ? '缺少单号' : text}/${(record.get('omsSapDocNo') == null || record.get('omsSapDocNo') == '') ? '缺少单号' : record.get('omsSapDocNo')}`}</span>);
                } else {
                  return (
                    <span>{`${(text == null || text == '') ? '缺少单号' : text}/${(record.get('omsSapDocNo') == null || record.get('omsSapDocNo') == '') ? '缺少单号' : record.get('omsSapDocNo')}`}</span>);
                }
              }}
            />
            <Column
              name="gbssPoNo"
              renderer={({text, record}) => {
                if (record.get('gbssPoNo') !== record.get('omsPoNo')) {
                  return (<span
                    style={{color: '#ff3fa4'}}>{`${(text == null || text == '') ? '缺少单号' : text}/${(record.get('omsPoNo') == null || record.get('omsPoNo') == '') ? '缺少单号' : record.get('omsPoNo')}`}</span>);
                } else {
                  return (
                    <span>{`${(text == null || text == '') ? '缺少单号' : text}/${(record.get('omsPoNo') == null || record.get('omsPoNo') == '') ? '缺少单号' : record.get('omsPoNo')}`}</span>);
                }
              }}
            />
            <Column
              ...
              align="right"
              renderer={({text, record}) => {
                if (record.get('gbssPoAmount') !== record.get('omsPoAmount')) {
                  return (<span
                    style={{color: '#ff3fa4'}}>{`${formatNumber(text)}/${formatNumber(record.get('omsPoAmount'))}`}</span>);
                } else {

                  return (<span>{`${formatNumber(text)}/${formatNumber(record.get('omsPoAmount'))}`}</span>);
            ...
            <Column
              ...
              align="center"
              renderer={({text, record}) => {
                if (record.get('gbssPoStatus') !== record.get('omsPoStatus')) {
                  return (<span style={{color: '#ff3fa4'}}>{`${text}/${record.get('omsPoStatus')}`}</span>);
                } else {
                  return (<span>{`${text}/${record.get('omsPoStatus')}`}</span>);
